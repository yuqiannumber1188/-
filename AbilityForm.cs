...
namespace cjpc
{
    public partial class AbilityForm : Form
    {
        ...
        public AbilityForm(MainForm _fm)
        {
            InitializeComponent();
            ...
            string sqlability = "select CLASSNAME,CLASSID from FXCLASS where PCLASSID = 10003 and SJC='" + labelsjc.Text + "'";


            dtability = dbclass.GreatDs(sqlability).Tables[0];

            abilitycount = dtability.Rows.Count;
            ...
            {
                temp = dtability.Rows[i][0].ToString();
            ...
            for (int i = 0; i < 9; i++) // 外层循环遍历行
            {
                ...
                {
                    if (i >= abilitycount || j >= abilitycount)
                    {
        ...
        {
            double[,] data = new double[abilitycount, abilitycount];

            for (int i = 0; i < abilitycount; i++) // 外层循环遍历行
            {
                for (int j = 0; j < abilitycount; j++) // 内层循环遍历列
                {
                    data[i, j] = 0;
                }
            }
        ...
        {
            string selecthtrw = "select SJC from Three_Weight_INFO where SJC='" + labelsjc.Text + "' and PCLASSID = 10003";
            string getdata = dbclass.GetOneValue(selecthtrw);

            if (getdata.Equals(""))
            {
                string[] insertthw = new string[abilitycount];
                try
                {
                    for (int i = 0; i < abilitycount; i++)
                    {
                        insertthw[i] = String.Format("Insert into Three_Weight_INFO(CLASSID,CLASSNAME,PCLASSID,SJC,WEIGHT) values ('{0}','{1}','{2}','{3}',{4})", int.Parse(dtability.Rows[i][1].ToString()), dtability.Rows[i][0].ToString(), 10003, labelsjc.Text, Double.Parse(lbWA[i].Text));
                    }
                }
                catch (FormatException)
                {
            ...
            {
                string[] updatethw = new string[abilitycount];
                try
                {
                    for (int i = 0; i < abilitycount; i++)
                    {
                        updatethw[i] = "update Three_Weight_INFO set WEIGHT = " + Double.Parse(lbWA[i].Text) + " where CLASSID = " + int.Parse(dtability.Rows[i][1].ToString()) + " and PCLASSID = 10003 and  SJC = '" + labelsjc.Text + "'";
                    }
                }
                catch (FormatException)
                {
