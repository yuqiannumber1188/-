...
namespace cjpc
{
    public partial class KnowledgeForm : Form
    {
        ...
        DataTable dtknowledge = new DataTable();
        ...
        public KnowledgeForm(MainForm _fm)
        {
            InitializeComponent();
            ...
            string sqlknowledge = "select CLASSNAME,CLASSID from FXCLASS where PCLASSID = 10001 and SJC='" + labelsjc.Text + "'";


            dtknowledge = dbclass.GreatDs(sqlknowledge).Tables[0];

            knowledgecount = dtknowledge.Rows.Count;
            ...
            {
                temp = dtknowledge.Rows[i][0].ToString();
            ...
            for (int i = knowledgecount; i < 9; i++)
            {
            ...
            for (int i = 0; i < 9; i++) // 外层循环遍历行
            {
                ...
                {
                    if (i >= knowledgecount || j >= knowledgecount)
                    {
        ...
        {
            double[,] data = new double[knowledgecount, knowledgecount];

            for (int i = 0; i < knowledgecount; i++) // 外层循环遍历行
            {
                for (int j = 0; j < knowledgecount; j++) // 内层循环遍历列
                {
                    data[i, j] = 0;
                }
            }
        ...
        {
            string selecthtrw = "select SJC from Three_Weight_INFO where SJC='" + labelsjc.Text + "' and PCLASSID = 10001";
            string getdata = dbclass.GetOneValue(selecthtrw);

            if (getdata.Equals(""))
            {
                string[] insertthw = new string[knowledgecount];
                try
                {
                    for(int i=0;i< knowledgecount;i++)
                    {
                        insertthw[i] = String.Format("Insert into Three_Weight_INFO(CLASSID,CLASSNAME,PCLASSID,SJC,WEIGHT) values ('{0}','{1}','{2}','{3}',{4})", int.Parse(dtknowledge.Rows[i][1].ToString()), dtknowledge.Rows[i][0].ToString(), 10001, labelsjc.Text, Double.Parse(lbWA[i].Text));
                    }
                }
                catch (FormatException)
                {
            ...
            {
                string[] updatethw = new string[knowledgecount];
                try
                {
                    for (int i = 0; i < knowledgecount; i++)
                    {
                        updatethw[i] = "update Three_Weight_INFO set WEIGHT = " + Double.Parse(lbWA[i].Text) + " where CLASSID = " + int.Parse(dtknowledge.Rows[i][1].ToString()) + " and PCLASSID = 10001 and  SJC = '" + labelsjc.Text + "'";
                    }
                }
                catch (FormatException)
                {
